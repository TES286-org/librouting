...
//!
//! - **UDP discovery transport** — one socket on the LDP port
//!   (§3.10.1: 646), joined to the all-routers multicast group
//!   (224.0.0.2) on every configured interface. Link Hellos are
//!   originated per interface every hold-time third (§3.5.2.1) with
//!   multicast TTL 1 and the interface's primary address as the source;
//!   targeted Hellos ride the same socket as ordinary unicast.
...
use lr_osroute::ospf_transport::interface_v4_addrs;
...
/// (RFC 5036 §3.5.2). IPv6 link discovery (ff02::2) belongs to the
/// RFC 7552 slice.
...
struct LdpInterface {
    ...
    /// Last link Hello on this interface (ms since daemon start).
    last_hello_ms: u64,
}
...
pub(super) fn run_ldp_daemon(cfg: &DaemonConfig, rid: RouterId) -> ExitCode {
    ...
    for spec in &cfg.ldp_interfaces {
        ...
        println!(
            "daemon: ldp interface {} — discovery on {} ({} address(es))",
            name,
    ...
    // Targeted peers: `ADDR` or `ADDR:PORT`. The engine deals in
    // addresses; a port override means that peer runs its LDP transport
    // on a different port (asymmetric deployments on shared hosts) —
    // Hellos and the TCP connect go there instead of the local port.
    ...
    for peer in &cfg.ldp_targeted {
        ...
        let (addr_part, port_part) = match spec.rsplit_once(':') {
            // A bare IPv6 (no brackets) has no port split; only accept
            // the split when both halves parse.
            Some((a, p)) => match (IpAddr::from_str(a), p.parse::<u16>()) {
                (Ok(addr), Ok(port)) => (addr, Some(port)),
                _ => match IpAddr::from_str(spec) {
                    Ok(addr) => (addr, None),
                    Err(_) => {
                        eprintln!("{}", bad());
                        return ExitCode::from(2);
            ...
            None => match IpAddr::from_str(spec) {
                Ok(addr) => (addr, None),
                Err(_) => {
                    eprintln!("{}", bad());
                    return ExitCode::from(2);
...
impl LdpDaemon {
    ...
    /// Feed every pending UDP datagram into the engine.
