...
|--------|------------------------------------------------------|--------------------------|--------------------------------------|
| 4271  | BGP-4                                                | ✓ core; FRR `bgp enforce-first-as` leftmost-AS check (W2.2, off by default); FRR `bgp default ipv4-unicast` activation posture (W2.1, on by default); FRR `allowas-in N` / BIRD `allow local as` per-peer AS-loop tolerance (W2.3, default 0 = reject any) | `lr-bgp::fsm`, `lr-bgp::message`, `lr-bgp::peer::{ipv4_unicast_active, local_as_tolerance}`, `lr-router::import_route`, `lr-policy::safety::SafetyNet::local_as_count` |
