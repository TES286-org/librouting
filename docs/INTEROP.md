...
PLACEHOLDER_NOT_FOUND `tests/interop/ospf_gr_bird.sh` | lr-daemon restarts × BIRD 2 helper over a veth pair (CI: jammy's 2.0.8; also verified against 2.17.x locally) | lr's Grace-LSA engages BIRD's helper mode ("started/finished graceful restart" in BIRD's log), BIRD retains 10.99.2.0/24 through the whole restart window — the graceful-shutdown flood services the protocol between rounds (ACKs the helper's in-flight LSA refreshes, keeps Hellos flowing), which a BIRD 2.0.8 helper's RFC 3623 §3.1 (2) check requires |
...
./tests/interop/ospf_sr_frr.sh              # needs FRR (or skips) — RFC 8667 slice 1; SRDB phase kernel-MPLS-gated
