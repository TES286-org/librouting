...
| OSPF GR x BIRD helper | `tests/interop/ospf_gr_bird.sh` | lr-daemon restarts × BIRD 2.17.5 helper over a veth pair | lr's Grace-LSA engages BIRD's helper mode ("started/finished graceful restart" in BIRD's log), BIRD retains 10.99.2.0/24 through the whole restart window |
