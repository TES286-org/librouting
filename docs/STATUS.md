...
| Filter DSL: perf baseline + import-pipeline bench | ✅ | `crates/lr-policy/benches/` (GitHub #19 P0) — `filter_eval` covers six shapes (3 original + `large_prefix_set` 100-entry / `large_community_set` 10-entry / `user_functions` two-call chain) under both engines; `import_pipeline` measures the daemon per-UPDATE cost (bytecode eval → Adj-RIB-In install → Loc-RIB install) at 100 / 1 000 / 10 000 routes under `trivial` and `realistic` filters; equivalence pinned on the bench-sized shapes by `vm_matches_interpreter_on_bench_shapes` (additive to the 27×4 table) |
