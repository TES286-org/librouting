...
| W3-extra — comprehensive MPLS | label codec (3032), BGP-LU dataplane mirror, LDP (5036) + dual-stack (7552) + transit allocation + loop detection + GR (3478), kernel mirror, SR-MPLS slices 1–3 (RFC 8665/8660/8661) | SR-MPLS complete; SRv6 slice 1 (data plane) + slice 2 (OSPFv3 SRv6 control plane, RFC 9513) landed — see [`ROADMAP.md` §W3-extra](ROADMAP.md#w3-extra--comprehensive-mpls-support-new) |
...
1. **SRv6 (RFC 8754 / RFC 8402 / RFC 8986)** — slice 1 (the
   ...
   Linux netlink. Slice 2 (control-plane extensions) has landed: the
   OSPFv3 SRv6 extensions of RFC 9513 (the repo previously cited
   RFC 9352, which is the IS-IS SRv6 sibling — corrected) — the
   codec, the per-node SRv6 database, locator routes in the v3 SPF
   and the fail-closed `ospf_srv6_receive` router gate. Still open:
   the End.X / LAN End.X SIDs (RFC 9513 §9 — they ride the RFC 8362
   E-Router-Link TLV, a later slice), BGP-LS / BGP SR Policy
   (RFC 9256 / 9430), and slice 3 (daemon `--srv6-locator` CLI +
   auto-origination) builds on slice 2.
...
2. **OSPFv3 daemon depth** — slice 1 landed: the daemon runs v3 end
   ...
   (0x4005) in the v3 route calculation, and graceful restart
   (RFC 5187). The SRv6 control plane is no longer part of this gap —
   RFC 9513 landed at the library level (see the OSPFv3 SRv6 row
   above); the remaining SRv6 daemon surface is slice 3
   (`[ospf] srv6` config + locator/End-SID/RI origination). The v3
   codec + LSA + SPF surfaces are in the capability tables above.