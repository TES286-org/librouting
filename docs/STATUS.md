...
1. **SR-MPLS (RFC 8660/8665)** — the wire codecs, SRGB/prefix-SID
   configuration, origination (slice 1, FRR-verified), the receive
   path (slice 2: per-node SRDB → SPF label attach → kernel AF_MPLS
   mirror) and the adjacency/mapping-server slice (slice 3: Adj-SIDs
   + LAN Adj-SIDs per §6, the Extended Prefix Range TLV / M-flag
   mapping-server shapes per §4, with `adj_sid` +
   `[[ospf.mapping_server]]` daemon configuration) have landed
   (W3-extra.5); the remaining SR-MPLS work is SRv6. RFC 9256
   (Segment Routing Policy) builds on that data plane.
