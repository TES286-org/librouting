...
| Codec state cleared on session reset | ✅ 🧪 | `BgpPeer::reset` rebuilds the `BgpCodec` (`BgpCodec::new().with_asn4(self.cfg.asn4)`) so partial frames from the previous transport are discarded before the new OPEN exchange — a stale `carryover` buffer would otherwise be decoded against the fresh session. BIRD drops partial bytes by freeing the socket in `bgp_close_conn`. Test: `crates/lr-bgp/tests/codec_reset.rs` (partial UPDATE, reset, re-establish, new UPDATE decodes cleanly) |
...
| RFC 4271 §4.4 negotiated keepalive cadence | ✅ 🧪 | the KEEPALIVE interval is capped at one-third of the negotiated hold time (RFC 4271 §4.4) rather than the configured value — `handle_open_in_opensent` computes `keepalive.min((negotiated_hold_time / 3).max(1))` and the Established rearm reuses the same derived value so every keepalive fires at the negotiated cadence; hold-time zero disables keepalives (the timer is never armed). BIRD `packets.c:923` derives the same `hold_time / 3` default. Test: `crates/lr-bgp/tests/negotiated_keepalive.rs` (30-second timer simulation across configured keepalive 0/1/30) |
| RFC 7947 route server mode | ✅ 🧪 | transparent egress to RS-client peers: AS_PATH is not prepended and NEXT_HOP is preserved when `topo.rs_client` is set (`advertise.rs` gates both rewrites on `&& !topo.rs_client`), matching BIRD `attrs.c:1997` (`if (p->is_internal || p->rs_client)` skips prepend) and BIRD's default `next_hop_keep=NH_ALL` for rs_client. Test: `crates/lr-bgp/tests/route_server_transparency.rs` (transparent vs. non-transparent AS_PATH + NEXT_HOP assertion, stored route unchanged) |
| RFC 9234 OTC / roles | ✅ | |
...
| Filter DSL: source spans + positioned diagnostics | ✅ | `lr-policy::filter::span` (issue #18 Phase 0) — every token, AST node and error carries a byte `Span`; `LineIndex` renders `line:col`, `render_snippet` draws a caret diagnostic; parse errors point at the offending token (incl. `UnknownFunctionCall` at the call); evaluation errors carry real positions in **both** engines with an engine-parity test (span table parallel to VM instructions, preserved through the peephole passes); the daemon and `lrctl filter compile` render positioned errors with snippets; `MAX_EXPR_DEPTH` recalibrated 128 → 108 to keep the recursion guard's debug-stack margin under the fatter AST nodes |
...
| Filter DSL: lazy span read in the VM dispatch loop | ✅ | `lr-policy::filter::eval` (GitHub #19 P7) — `run_code` reads the source span lazily inside the fallible arms instead of once per dispatch at the loop top; infallible arms (Push / Jump / Accept / Reject / Return / Pop / PushScope / PopScope / StoreTmp) pay zero span cost, fallible arms pay one direct-indexed `spans[ip]` load (the `spans` slice is parallel to `code` and `ip` is loop-bounds-checked). Threads `spans: &[Span]` as a parameter to `run_code` alongside `code`, which also fixes a latent bug where a user-function body indexed the *outer filter's* span table (`cf.spans`) instead of its own (`f.spans`) — `call_compiled_function` now passes `&f.spans`, so errors inside a function body point at the function's own source; `vm_large_community_set/hit_last` −0.60 % (p = 0.00), `vm_large_community_set/miss` −0.38 % (p = 0.00); 1 new test pins the function-body span fix |
...
| OSPFv3 intra-area SPF (§4.8) | ✅ 🧪 | `run_spf_v3`: Network vertices keyed (DR Router ID, DR Interface ID); next hops are (link-local, outgoing Interface ID) pairs — a direct p2p neighbor's link-local resolves from its Link-LSA (LS ID = the Neighbor Interface ID of our link), routers on a directly attached transit network resolve through the back-link transit entry, deeper vertices inherit; FRR `ospf6_lsdesc_backlink` bidirectional check; prefixes arrive via Intra-Area-Prefix-LSAs — only NU-marked prefixes are excluded from the unicast calculation (§A.4.1; LA-marked prefixes are the advertising router's own local address and remain reachable, matching BIRD `rt.c`); the Router-LSA options of every router are exposed for 0x2004 origination (§4.4.3.5); routes publish as `Protocol::Ospfv3` in the v6-unicast family |
...
| RFC 8966 §4.5.2 prefix compression + AE 4 v4-over-v6 (RFC 9229 §2.4) | ✅ 🧪 | per-packet `PrefixCache` (per-AE, like BIRD's parse state) expands omitted-octet Updates — BIRD compresses its v6 announcements routinely, and a receiver that ignores the compression learns *corrupted* prefixes; AE 4 Updates announce IPv4 destinations over the IPv6 next hop on v6-only links; verified against BIRD 3.3.2 in `tests/interop/babel_dualstack_bird.sh` |
...
| Dual-stack announcements + Loc-RIB preference + BIRD interop | ✅ 🧪 | the v6 transport carries BOTH families (babeld's shape: NextHop(AE 1, our v4 address) before AE 1 Updates — BIRD/babeld never listen on 224.0.0.111); v6-only links use AE 4 (RFC 9229); a Babel-learned route for a prefix the operator ALSO configured as a static no longer displaces it (static distance 1 < babel 120 — the "announced aggregates stop propagating" bug); self-router-id echoes ignored (BIRD's guard); Route/Seqno Requests answered with an immediate announcement (§3.2.6); announcement seqno seeded randomly (babeld parity) with seqno-request recovery; `tests/interop/babel_dualstack_bird.sh`: full production topology — v4-on-v6 propagation, compressed-prefix decoding, real ICMP forwarding through babel-installed kernel routes, a BGP session to a Babel-only-reachable peer, pcap TLV analysis, shutdown withdrawal |
| Multi-interface sessions + per-interface parameters (ROADMAP-v3 D1) | ✅ 🧪 | `[[babel.interface]]` glob patterns resolve against the system's interfaces (first match wins, BIRD semantics); each match gets its own session, router-id, socket pair (unicast address-bound with SO_BINDTODEVICE + wildcard multicast listener isolated via `IP_MULTICAST_ALL=0` and the per-interface membership), `hello_interval_ms` / `update_interval_ms` / `rxcost` / `rtt_cost`+`rtt_min_us`+`rtt_max_us` / `next_hop_ipv4` / `next_hop_ipv6` / `extended_next_hop` / `port` / `group` / `check_link` (1 s poll, withdraw-on-down, announce-on-return); dual-stack interfaces announce on both transports (§4.1; the v4 transport carries IPv4 destinations only); routes learned on one interface are re-advertised on the others with the origin's (router-id, seqno) and the interface cost added (§3.7.5), per-session split horizon, and vanished claims retracted with infinity-metric Updates (§3.5.5); Hello seqno advances per Hello (§3.4.1) while the Update seqno tracks content changes (§3.7.1); `[[babel.key]] interface` scopes RFC 8967 keys per interface; `tests/interop/babel_multihop.sh` (three speakers, three namespaces: transit both ways, kernel FIB install/withdraw, check-link withdrawal end-to-end, reconvergence) + `babel_multi_nic.sh` |
...
| Native `.lr` config DSL + `to-dsl` converter (issue #18 Phase 2) | ✅ 🧪 | The declarative DSL specified in `docs/config_dsl_grammar.md`: blocks with identity arguments (`peer "core-1" { … }`), unit suffixes whitelisted per (section, key) (`hold_time 90s;`, babel-interval milliseconds, `max_prefixes 100k`), verbatim filter bodies between braces (no string escaping), cycle-checked `include` with per-file diagnostics. Lowering drives the same `apply_config_key` dispatch as the TOML subset parser — one fail-closed key schema, no drift; `Dialect::Lr` in `--config-dialect` + content detection. `lr-daemon config to-dsl <file>` renders the IR deterministically and refuses unrepresentables (parse warnings, filter descriptions); kitchen-sink round-trip covers every key of every section, shipped template included. **Phase 3 landed**: the daemon runs natively on `.lr` everywhere — content-detected at startup, SIGHUP/API reload re-resolves the dialect through the shared `Dialect::from_flag` parser (reload no longer fails with "unknown config dialect 'lr'"), and `templates/daemon.lr` ships as the DSL-first reference template (IR-equal to the TOML twin, pinned by unit + e2e golden tests; `docs/`, README and examples switched DSL-first). **Phase 4 landed**: the TOML deprecation window is announced by the code — daemon startup, SIGHUP/API reload and `config check` print a `config deprecation` notice when the resolved dialect is TOML (a policy announcement, not a parse warning; `to-dsl` stays silent by design — it is the migration tool; TOML supported through 1.x, removed in 2.0) |
