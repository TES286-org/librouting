...
**Status:** landed — ~~D9.2 (filter DSL EBNF grammar)~~, ~~D9.6
(`docs/ffi_design.md`)~~, ~~D9.3 (`CONTRIBUTING.md`)~~, ~~D9.4
(`SECURITY.md`)~~, ~~D9.5 (`CHANGELOG.md`)~~, ~~D9.1 (Filter DSL
chapter + thread model + performance chapter in `ARCHITECTURE.md`)~~
and ~~D9.1 (`instance.rs` internal-structure chapter)~~ all landed.
Tracks `docs/`.
...
(D9.1), the thread-model + lock-strategy chapter (D8.6,
`ARCHITECTURE.md` "Performance characteristics"), the formal EBNF
(D9.2, `docs/filter_dsl_grammar.md`), the FFI design doc (D9.6,
`docs/ffi_design.md`), the contributor checklist (D9.3,
`CONTRIBUTING.md`), the security policy (D9.4, `SECURITY.md`), the
structured changelog (D9.5, `CHANGELOG.md`, 1 384 lines covering
`rc.1` → `rc.4`) and the `instance.rs` internal-structure chapter
(D9.1, `ARCHITECTURE.md` "Router instance internals") all landed.
D9 is now complete.
...
1. ~~**`docs/ARCHITECTURE.md` expansion.** Filter DSL chapter (AST
   ...
   piece — the `instance.rs` internal-structure chapter (Loc-RIB
   data structures, reselect path, export hook ordering,
   redistribution tracking, `redistributed_bgp` BTreeMap design,
   OSPF / Babel runtime structs) — landed as the new "Router
   instance internals" section in `docs/ARCHITECTURE.md`.~~ The
   chapter covers: the three kinds of `DefaultRouter` state
   (per-session runtime / Loc-RIB + Adj-RIBs / cross-protocol OSPF
   + misc) with a field map; the per-protocol runtime structs
   (`OspfRuntime`, `OspfAreaState`, `BabelRuntime`, `RuntimeDelta`)
   and why the LSDB is per-area while the Babel runtime is
   session-scoped; the four Loc-RIB contribution maps
   (`originated` / `static_routes` / `direct_rib` /
   `redistributed_bgp`) and how `reselect(key)` merges them; the
   six-stage export hook ordering (split horizon → protocol gate →
   RFC 8212 → export hooks → Add-Path vs single-path → Adj-RIB-Out
   diff) with the two-phase collect/transmit split that the borrow
   checker forces; the per-destination MRAI queue and why
   withdrawals bypass it; the `redistribute_route` feedback-loop
   guard and the `MetricPolicy::Add(N)` fixpoint it protects
   against; and the OSPF top-level state map (areas / published /
   externals / stable LS IDs / NSSA translations / virtual links /
   Grace-LSA dedup) including why the stable-LS-ID maps exist (v3
   LS IDs carry no addressing semantics) and why Grace-LSA dedup
   keys on the (seq, age, checksum, length) tuple rather than on
   sequence alone.
...
Filter DSL chapter ~165; ~~the `instance.rs` deep-dive landed as
a ~220-line "Router instance internals" section in
...
**Status:** landed. Tracks `lr-ospf` (+ `lr-router`, `lr-cli`). The
research doc `docs/research/E-LSA-DESIGN.md` predated the
implementation and carried a wrong LS-type/TLV table plus a
...
1. **Codecs** (`crates/lr-ospf/src/lsa/e_v3.rs`): the eight
   ...
   0xA022 (34), E-Inter-Area-Prefix 0xA023 (35), E-Inter-Area-Router
   0xA024 (36), E-AS-External 0xC025 (37, AS scope), E-Type-7 0xA027
   (39), E-Link 0x8028 (40, link scope), E-Intra-Area-Prefix 0xA029
   (41); fc 38 stays unallocated — with the RFC 3630 TLV framing and
   the §5/§6.3 malformed/skip rules. 13 unit tests pin the wire
   shapes against the RFC figures. Origination helpers for all
   eight shapes (sequence machinery shared with the legacy v3
   originators).
2. **Reception + origination** (spf.rs / external.rs / instance.rs /
   daemon_ospf3.rs): the receiver decides — `run_spf_v3_extended` /
   `summary_routes_v3_extended` / `external_routes_v3_extended`
   prefer a speaker's E-LSAs (per-speaker, first E-instance wins) and
   admit the E inter-area/external forms, while the legacy paths
   stay byte-identical (§6.2: E-LSAs stored + re-flooded, never used
   for the calc); `ospf_area_accepts` classifies the E forms, 0xC025
   re-floods at AS scope, the RFC 3623 topology bookkeeping counts
   them. The daemon knob `[ospf] extended_lsas` (OSPFv3-only,
   fail-closed) switches origination to the E-Router/E-Network/
   E-Link/E-IAP forms (ABR/ASBR origination stays legacy — §6.1
   migrates areas individually). The two-daemon lab
   `tests/interop/ospf6_e_lsa.sh` makes legacy origination impossible,
   so convergence proves the E-LSA path end to end; the
   `ospf6.sh`/`ospf6_broadcast.sh` regressions pin the default.
3. **SRv6 End.X** (`lsa/srv6.rs` / `srv6db.rs` / daemon): the §9.1
   End.X (registry type 31) and §9.2 LAN End.X (32) sub-TLV codecs
   ride the E-Router-Link TLV's sub-TLV region (§10 SID Structure
   as type 30); the srv6db projects `Srv6EndXSid`s gated on §9
   locator containment + algorithm match (multiple instances kept —
   load-balancing); the daemon originates the p2p form per
   `[[ospf.interface]] srv6_end_x` (fail-closed inside a configured
...
OSPFv3 Extended LSA that turns fixed-length LSA types into variable
TLV structures, allowing larger Router-ID / Link-ID spaces. SRv6
End.X SIDs (RFC 9513 §8) need E-LSA to carry SIDs long enough.
...
2. ~~**Phase 1 — IR extraction.**~~ Landed in five slices
   (`516ad9e` IR equality, `5ca7f36` shared load entry,
   `6c8b999` inline-comment fix, `9a95201` golden tests,
   `1d587ca` e2e). The typed IR is `DaemonConfig` itself: every
   section spec struct already derived `PartialEq` — the root
   `DaemonConfig` gained it, so two parsed configurations are
   comparable for semantic equality and the IR-equality property is
   directly executable. Three surfaces load through one
   `daemon_config::load_config_file` entry point (read file →
   resolve dialect → parse → stamp `config_path`/`config_dialect`):
   daemon startup (`parse_args`), SIGHUP / API `reload` and the new
   `lr-daemon config check` validator, so the set of configs the
   daemon accepts is exactly the set `check` accepts and the three
   cannot drift apart. `config check` (riding the daemon binary like
   `translate`/`yang`, no lib.rs split) runs the shared load plus
   `finalize` — template inheritance, cross-section name references
   (peer → peer-template, route-map → prefix-list), RPKI interval
   bounds — and prints the resolved view (dialect, protocol set,
   peers, networks, the policy bank, babel/ospf/roa/redistribute
   counts, every parse warning) without starting the daemon;
   exit 0 valid / 1 invalid / 2 usage. The golden tests pin the
   equality contract Phase 2 will validate the DSL frontend against:
   parse determinism (the shipped template twice → equal IRs),
   variant equivalence (the two top-level protocol-set spellings and
   re-ordered keys inside a table → equal IRs), inequality (one
   differing field separates IRs; `[[peer]]` order is semantic —
   peers match by index), and the shipped template as the standing
   structured golden file. Eight e2e tests cover the check paths
   (valid multi-section TOML, the golden template, positioned parse
   errors, a finalize-only cross-section failure, BIRD detection,
   forced dialect, usage errors, missing file). The golden test run
   also exposed and fixed a real parser bug: TOML inline comments
   (`entry = 20      # …`, present in `templates/daemon.toml`
   itself) were rejected — `parse_toml_subset` now strips them with
...
| D9        | landed                | —     | Filter DSL architecture chapter in `docs/ARCHITECTURE.md` (AST / Pratt parser / evaluator / `FilterContext` / bytecode VM + peephole / BIRD `f_line` comparison), formal EBNF grammar + corpus test, `docs/ffi_design.md`, the 173-line `CONTRIBUTING.md`, the 93-line `SECURITY.md` (rc.4 version pin, 90-day coordinated disclosure, threat model), the 1 384-line `CHANGELOG.md` (rc.1 → rc.4) and the new ~220-line "Router instance internals" chapter in `docs/ARCHITECTURE.md` (DefaultRouter state shape, per-protocol runtime structs, Loc-RIB contribution maps + reselect, six-stage export hook ordering, MRAI per-prefix queue, redistribution tracking + feedback-loop guard, OSPF top-level state map) all landed; D9 is complete |
