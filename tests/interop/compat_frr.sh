#!/usr/bin/env bash
# FRR interop test for the compat surface (W5.4): lr-daemon runs an
# FRR configuration file NATIVELY (`lr-daemon --config frr.conf`) and
# exchanges real BGP with FRR's bgpd.
#
#   lr-daemon from frr.conf (AS64512, connector, network 203.0.113.0/24)
#        ↑↓ TCP on 127.0.0.1
#   FRR bgpd (AS64514, listener, network 198.51.100.0/24)
#
# bgpd runs standalone (no zebra, no kernel) on unprivileged ports, so
# the test works as any user. The routing table is read back over the
# vty TCP interface (same harness as frr.sh).
#
# Address notes (as in frr.sh): FRR rejects loopback next hops as
# martian, so lr's config pins its egress next hop via
# `neighbor … update-source 192.0.2.1` and FRR pins its export next
# hop to 192.0.2.20 via a route-map. Neither needs to be assigned to
# an interface.
#
# Success criteria:
#   1. The session reaches Established (lr log).
#   2. FRR's BGP table contains 203.0.113.0/24 learned from lr-daemon.
#   3. lr-daemon's log shows 198.51.100.0/24 installed from FRR.
#   4. The frr-dialect-defaults warning proves the compat path ran and
...
#   PORT      bgpd listen port (default 17997)
set -euo pipefail
cd "$(dirname "$0")/../.."
...
# The subject: lr-daemon runs the FRR-shaped config DIRECTLY — no
# conversion step. FRR's own defaults (enforce-first-as on) carry over
# through the compat surface; `lr: api-socket` is the lr extension.
cat >"$OUT/lr.frr.conf" <<EOF
frr version 10
frr defaults traditional
!
router bgp 64512
 bgp router-id 10.0.0.1
 neighbor 127.0.0.1 port $PORT remote-as 64514
 neighbor 127.0.0.1 update-source 192.0.2.1
...
echo "== starting FRR bgpd (AS64514, listener :$PORT) =="
"$BGPD" -f "$OUT/bgpd.conf" -i "$OUT/bgpd.pid" \
    -Z -n -S \
    -p $PORT -A 127.0.0.1 -P $VTY_PORT --vty_socket "$OUT/vty" \
...
# Wait for the vty and verify it answers as OUR bgpd.
vty_ok=1
...
echo "== starting lr-daemon from the FRR dialect config (AS64512, connector) =="
"$BIN" --config "$OUT/lr.frr.conf" >"$OUT/lr.log" 2>&1 &
LR_PID=$!
trap 'kill ${BGPD_PID:-} ${LR_PID:-} 2>/dev/null || true' EXIT
...
for i in $(seq 1 180); do
    ...
    if [ $ok_compat -ne 0 ] && [ -S "$OUT/api.sock" ] \
        && grep -qF "frr dialect defaults applied" "$OUT/lr.log" 2>/dev/null; then
        ok_compat=0
    fi
    ...
    if [ $ok_frr -eq 0 ] && [ $ok_lr -eq 0 ] && [ $ok_compat -eq 0 ]; then
        break
    fi
done
...
fail=0
if [ $ok_frr -ne 0 ]; then
    echo "FAIL: FRR did not learn 203.0.113.0/24 from the compat-mode lr-daemon"
    fail=1
fi
if [ $ok_lr -ne 0 ]; then
...
if [ "$fail" -eq 0 ]; then
    echo "PASS: FRR compat interop — lr-daemon ran the FRR config natively and exchanged routes with bgpd"
fi
exit $fail
