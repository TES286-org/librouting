...
Three interop scripts need kernel features the host may not provide:
...
kernel, the MPLS modules via `modprobe` as root), so the workflows run
the tests natively. On a workstation, container, or restricted sandbox
without the right kernel or without root, this directory provides a
repeatable fallback: a minimal initramfs VM (no disk image, no
installation) where the tests run as real root against a kernel that
...
1. `build_initramfs.sh` assembles a tiny rootfs with the guest
   ...
   `lr-daemon`, and the three scripts, then packs it with
...
2. `run_vm.sh` boots `qemu-system-x86_64` with `-kernel` / `-initrd`
   ...
   three scripts passed inside the guest.
